<template>
  <q-item class="bg-white q-py-none q-mb-sm q-px-sm justify-between shadow-3" style="border-radius: 8px">
    ...
    <q-item-section class="items-center col-2">
      <q-icon color="grey" name="las la-chevron-circle-right" size="xs"></q-icon>
    </q-item-section>
    <q-item-section class="justify-center">
      <q-item-label class="text-bold text-black currency-rate q-mt-none text-right">{{pairAmountForBaseAmount}}
      </q-item-label>
      <q-item-label class="currency-rate-subtitle q-mt-none text-grey-7 text-right">1 {{baseCurrencyName}} =
        {{baseToPairRate}} {{pairCurrencyName}}
      </q-item-label>
      <q-item-label class="currency-rate-subtitle text-grey-7 text-right">1 {{pairCurrencyName}} = {{pairToBaseRate}}
        {{baseCurrencyName}}
      </q-item-label>
    </q-item-section>
  </q-item>
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      baseCurrencyAmount: {type: String, default: "0.00"},
      pairCurrencyName: {type: String, default: ""},
      pairCurrencyRate: {type: Number, default: 0}
    },
    computed: {
      pairAmountForBaseAmount() {
        return Decimal.mul(this.baseCurrencyAmount || 0, this.pairCurrencyRate).toFixed(2)
      },
      baseToPairRate() {
